...
def products(request):
    ...
    if filter_form.is_valid():
        if filter_form.cleaned_data.get('name'):
            products = products.filter(name__icontains = filter_form.cleaned_data.get('name'))

        if filter_form.cleaned_data.get('min_price'):
            products = products.filter(price__gte = filter_form.cleaned_data.get('min_price'))

        if filter_form.cleaned_data.get('max_price'):
            products = products.filter(price__lte = filter_form.cleaned_data.get('max_price'))

        if filter_form.cleaned_data.get('categories'):
            products = products.filter(categories__in = filter_form.cleaned_data.get('categories')).distinct()
...
def place_order(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # create order items for newly creted order
            for cart_item in cart_products:
...
def khalti_payment(request, order_id):
    ...
    try:
        ...
        payment_obj, created = Payment.objects.get_or_create(purchase_order_id=purchase_order_id, order=order, amount=order.total)
        ...
        headers = {
            'Authorization': 'key b81c074a78ce48f09de52b2ebf768c9e',
...
def khalti_payment_response(request):
    ...
    try:
        payment = Payment.objects.get(pidx=pidx, purchase_order_id=purchase_order_id, amount=amount)
        ...
        try:
            ...
            messages.success(request, "Payment done, please wait for product to reach your doorsteps")
            return redirect("store:order_page")
